...
namespace Create_Team
{
  class Program
  {
    ...
    static async Task Main(string[] args){
      ...
      {
        var parameters = new TMCreateTeamRequest();
        parameters.@public = true;
        parameters.name = "Fun team";
        parameters.description = "Let's chit chat here";

        var member1 = new TMCreateTeamRequestMembers();
        member1.email = "member.1@gmail.com";
        var member2 = new TMCreateTeamRequestMembers();
        member2.email = "member.2@gmail.com";
        parameters.members = new TMCreateTeamRequestMembers[] { member1, member2 };

        var response = await restClient.TeamMessaging().V1().Teams().Post(parameters);
