...
app.post('/callback', function (req, res) {
  ...
  if (validationToken) {
    ...
    if (req.body.ownerId == req.body.body.creatorId) {
      ...
      send_message( `I do not understand ${req.body.body.text}`, req.body.body.groupId )
    }
  }
  res.statusCode = 200;
  res.end('');
...
async function send_message( msg, group ) {
  console.log("Posting response to group: " + group);
  try {
    let endpoint = `/team-messaging/v1/chats/${group}/posts`
    let bodyParams = {
          text: msg
        }
...
async function send_card( card, group ) {
  console.log("Posting card to group: " + group);
  try {
    let endpoint = `/team-messaging/v1/chats/${group}/adaptive-cards`
    let resp = await platform.post(endpoint, card)
    let jsonObj = await resp.json()
    console.log(jsonObj)
  }catch(e) {
    console.log(e.message)
...
async function update_card( group, card, content ) {
  console.log("Updating card...");
  try {
    let endpoint = `/team-messaging/v1/chats/${group}/adaptive-cards/${card}`
    let resp = await platform.put(endpoint, content)
  }catch(e) {
    console.log(e.message)
  }
}
