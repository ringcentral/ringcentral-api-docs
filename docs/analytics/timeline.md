...
This API users to get the view of the count of calls (counter) or time spent on calls (timer) broken down by timeframes for specified data scopes. 
...
Compared to aggregate end point, one additional point that needs to be answered in timeline endpoint is the TimeFrame (preferred interval). 

## Composing a request to the Timeline API
